...
| 阶段 | 日期（UTC） | 提交 | 榜单 |
|---|---|---|---|
| Playground | 注册起至颁奖日 | decisions.csv 或智能体程序包，每队每天 50 次 | 仅供参考 |
| 线上比赛 | 2026-10-04 16:00 至 2026-10-07 15:59（UTC+8 为 10 月 5–7 日） | decisions.csv 或智能体程序包，每队每天 10 次 | 决定奖项 |
...
1. **结果文件。** 列为 `decision_id, slot_id, action, tile_id, program, request_id, reason` 的 `decisions.csv`，由你在本地运行智能体生成。两个阶段都接受，由冻结的评分器即时评分。比赛场景的天气不公开，平台用自己手上的天气真值给你的决策序列评分。
