"""Scorer parity, protocol and sandbox tests for the worker (no network)."""
...
def test_reference_report_matches_frozen_scorer(tmp_path):
    from worker.scoring import score_decisions
...
def test_invalid_csv_is_rejected(tmp_path):
    from worker.scoring import InvalidSubmission, score_decisions
...
def test_generator_is_deterministic(tmp_path):
    from worker.scenarios import generate_scenario_files, validate_scenario_files
...
def test_agent_crash_is_reported(tmp_path):
    from worker.runner import AgentRunError
...
def test_zip_slip_rejected(tmp_path):
    import zipfile
    from worker.runner import AgentPackageError, prepare_agent_dir
    z = tmp_path / "evil.zip"
