-- Coverage-uniformity bonus: carry the new score-report-v3 fields end to end.
-- The scorer gained coverage_bonus = coverage_bonus_weight * base_science * Jain_evenness (weight defaults to 0,
-- so practice scenarios keep scoring identically). Surface the term on evaluations, submissions and the
-- leaderboard so the site can itemise every point of the total.

alter table public.submissions add column if not exists coverage_bonus double precision;
alter table public.submissions add column if not exists coverage_evenness double precision;

alter table public.evaluations add column if not exists coverage_bonus double precision;
alter table public.evaluations add column if not exists coverage_evenness double precision;
